 ...
 */
class Rohde_AdvancedCms_Adminhtml_AdvancedcmsController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function deleteAction()
    {
        ...
        if ($id = $this->getRequest()->getParam('page_id')) {
            ...
            try {
                ...
                // $this->_redirect('*/*/');
                $this->_redirectUrl(Mage::helper('adminhtml')->getUrl('adminhtml/cms_page/index'));
                return;
                ...
                $this->_redirect('admin/cms_page/edit', array('page_id' => $id));
                return;
            }
        }
        // display error message
    ...
    public function massDeleteAction()
    {
        ...
        if(!is_array($pageIds)) {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('rohde_advancedcms')->__('Unable to find pages to delete.'));
        } else {
            try {
                foreach ($pageIds as $pageId) {
                    Mage::getModel('cms/page')->setId((int)$pageId)->delete();
                }
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('rohde_advancedcms')->__('Total of %d record(s) were deleted.', count($pageIds))
                );
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
            }
    ...
    {
        $pageIds = (array)$this->getRequest()->getParam('page_ids');
        ...
        try {
            foreach ($pageIds as $pageId) {
                switch ($status) {
                    case 1:
                        Mage::getModel('cms/page')->load($pageId)->setData('is_active', 1)->save();
                        break;
                    case 2:
                        Mage::getModel('cms/page')->load($pageId)->setData('is_active', 0)->save();
                        break;
                }
            }
    ...
    /**
     * Update page(s) Column
     *
     */
    public function massColumnAction()
    {
        $pageIds = (array)$this->getRequest()->getParam('page_ids');
        $storeId = (int)$this->getRequest()->getParam('store', 0);
        $column = (int)$this->getRequest()->getParam('column');
        ...
        try {
            foreach ($pageIds as $pageId) {
                ...
                switch ($column) {
                    case 1:
                        Mage::getModel('cms/page')->load($pageId)->setData('root_template', 'empty')->save();
                        break;
                    case 2:
                        Mage::getModel('cms/page')->load($pageId)->setData('root_template', 'one_column')->save();
                        break;
                    case 3:
                        Mage::getModel('cms/page')->load($pageId)->setData('root_template', 'two_columns_left')->save();
                        break;
                    case 4:
                        Mage::getModel('cms/page')->load($pageId)->setData('root_template', 'two_columns_right')->save();
                        break;
                    case 5:
                        Mage::getModel('cms/page')->load($pageId)->setData('root_template', 'three_columns')->save();
                        break;
                }
            }
            ...
            $this->_getSession()
                ->addException($e, $this->__('An error occurred while updating the page(s) column.'));
        }
    ...
    /**
     * inline ajax page title edit/save
     */
    public function savePageTitleAction()
    {
        ...
        try {
            $pageModel->setData('title', trim($value))->save();
        } catch (Exception $e) {
            echo $e->getMessage();
        }
        echo $pageModel->getData('title');

    }

    /**
     * inline ajax page url edit/save
     */
    public function savePageUrlAction()
    {
        $request = $this->getRequest();
        $editorId = $request->getParam('editorId');
        $value = $request->getParam('value');
        ...
        $pageModel = Mage::getModel('cms/page')->load($pageId);
        try {
            $pageModel->setData('identifier', trim($value))->save();
        } catch (Exception $e) {
            echo '<span style="color:red;">'.$e->getMessage().'</span>';
        }
        echo $pageModel->getData('identifier');
