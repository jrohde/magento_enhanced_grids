 ...
 */
class Rohde_AdvancedCms_Adminhtml_AdvancedblockController extends Mage_Adminhtml_Controller_Action
{
    /**
     * Init actions
     *
    ...
    public function massDeleteAction()
    {
        ...
        if(!is_array($blockIds)) {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('rohde_advancedcms')->__('Unable to find blocks to delete.'));
        } else {
            try {
                foreach ($blockIds as $blockId) {
                    Mage::getModel('cms/block')->setId((int)$blockId)->delete();
                }
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('rohde_advancedcms')->__('Total of %d record(s) were deleted.', count($blockIds))
                );
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
            }
